    ...
    for (let i = 0, len = key.length; i < len; i++) {
      ...
      if (brackets.has(key[i]) && key[i - 1] !== "\\") { // \( or \{ or \[
        ...
        if (k === "[") {
          ...
          placeholders.push({pattern, target, next, optional, repeatable, nodes})
        }
        current = next
    ...
    for (let {pattern, target, next, optional, repeatable, nodes} of placeholders.reverse()) {
      ...
      if (repeatable) {
        if (optional) { // *
          ...
          Object.assign(sub_trie.trie, second_trie.trie)
          Object.assign(second_trie.trie, next)
          next = second_trie.trie
        }
      }
