...
/**
 * Create a DOM Element that corresponds to a Virtual DOM Node
...
/**
 * Change a DOM Element to equal a Virtual DOM Node (Internal)
 * @param {VirtualNode} virtual_node
 * @param {Node|Element} dom_node
 * @param {number} level
 * @returns {Node|Element} (the mutated dom_node)
 */
function _merge(virtual_node, dom_node, level) {
  ...
  if (virtual_node.children) {
    for (let i = 0, n = virtual_node.children.length; i < n; i++) {
      ...
      if (dom_child) { // morph dom_child to match virtual_child (recursively)
        merged_children.push(_merge(virtual_child, dom_child, level++))
...
export function merge(virtual_node, dom_node) {
  let mutated = _merge(virtual_node, dom_node, 0)
  conclude()
  return mutated
}
